...
class _GaugeChartState extends State<GaugeChart>
  ...
  Future<void> _initAsync() async {
    ...
    final bool isExplicitSolo = widget.sessionType == 'solo';
    if (isExplicitSolo) {
      setState(() {
        isSolo = true;
        _showOverlay = false;
    ...
    if (!isExplicitSolo && _isHost!) {
      createSession();
    }

    if (!isExplicitSolo && !_isOnline!) {
    ...
    if (!isExplicitSolo && !_isOnline! && _isHost!) {
      setState(() {
        _showOverlay = false;
        isSolo = true;
      });
    ...
    if (!isExplicitSolo && _isOnline!) _listenForGuestJoin();
